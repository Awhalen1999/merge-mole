...
/// native size, review/CI state). The bottom half is the AI section, which is
/// driven entirely by the single `verdict` value — the card branches on it once
/// and never carries separate layouts per AI mode.
struct PRCard: View {
    let pr: PullRequest
